...
const HomeData = ({e}) => {
  return (
    ...
    <div className="h-40 flex justify-center overflow-hidden items-center rounded-xl">
       <img src={e?.image_url} alt="recipe item" className="block w-full" />
    </div>
    <div>
      {/* <span className="text-sm text-cyan-700 font-medium">{index.publisher}</span> */}
      <h3 className="font-bold text-2xl truncate text-black">{e.title}</h3>
      <Link to={`/recipe-item/${e.id}`}
